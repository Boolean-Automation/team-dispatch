...
time-track in-app; an embedded Claude window sits next to each ticket; admins get
...
- **Companion** — a lightweight process installed on each SE's machine. Hosts the
  embedded Claude window by running the SE's own Claude Code over a PTY.
- **Embedded Claude Window** — an xterm.js terminal in the dispatch UI, wired to
  the Companion. Runs on the SE's machine and subscription. Opens `claude` at the
  `boolean-knowledge` repo root with the Ticket + Account injected as context.
...
See `docs/adr/` for the six architecture decision records.

- ADR-001 — AI delivery via local Companion; BYO-subscription preserved.
...
- **Move 1 — cost** — real Pylon bill vs projected internal cost, redone with
  model-tiered Anthropic spend (interactive window rides SE subscriptions;
  classification + automation ride a Boolean Console org).
...
- **Phase 1 — the spine.** Slack ingestion → Tickets → unified dashboard → status
  ladder → in-app reply with Slack write-back. The replacement for the current
  workflow. Clerk auth, four-entity schema, client registry, routing, the
  `dispatch` MCP, undo-everywhere. A Claude Design pass on the unified dashboard
  sits between Phase 1 spec and Phase 1 dev.
- **Phase 2 — the AI layer.** Companion, embedded xterm.js terminal,
  boolean-knowledge context injection, server-side fallback engine, follow-up and
  closeout AI drafting. Gated on the Companion spike.
- **Phase 3 — time + economics.** In-app clock in/out, billable toggle, AI
  clockout-description generator, Clockify API sync, the per-SE hours bar chart
  (1.5h default per Ticket, SE-adjustable), §9.4 per-client effort capture.
- **Phase 4 — admin.** Chris/Cody KPI tabs, response + resolution time, client
  health (formula owned by Norman), per-SE performance.
